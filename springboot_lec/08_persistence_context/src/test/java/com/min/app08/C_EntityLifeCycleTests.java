...
class C_EntityLifeCycleTests {
  ...
  void 영속_test() {
    ...
    ┌---------- Persistence Context ----------┐   ┌-----------------------------------┐  
    │                                         │   │                                   │
    │           Menu(menuCode == 1)           │<--│        SELECT FROM WHERE          │
    │                                         │   │                                   │
    └-----------------------------------------┘   └-----------------------------------┘
     */
  ...
  void 준영속_merge_update_test() {
    ...
    // merge() 메소드 동작 순서
    // 1. foundMenu 엔티티의 @Id(menuCode = 1) 값으로 영속성 컨텍스트의 "1차 캐시"에서 엔티티를 조회합니다. "1차 캐시"에 없으면 DB 에서 조회하고 조회 결과 엔티티를 "1차 캐시"에 저장합니다. DB 에서 조회가 안 되면 새로운 영속 엔티티를 생성해서 반환합니다.
    // 2. foundMenu 엔티티는 영속성 컨텍스트의 "1차 캐시"에서 조회가 됩니다.
    // 3. "1차 캐시"에서 조회한 엔티티에 준영속 엔티티 foundMenu 의 값을 병합한 영속 엔티티를 반환합니다.
    Menu mergedMenu = entityManager.merge(foundMenu);
    
    // find() 결과 엔티티와 mergedMenu 엔티티는 동일하기 때문에 아래 두 테스트는 모두 통과합니다.

...
public class SecurityConfig {
  ...
  SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    ...
    http.authorizeHttpRequests(auth -> {
      auth.requestMatchers("/", "/user/signup", "/auth/login").permitAll();  // requestMatchers 에 등록된 모든 주소는 누구나 접근할 수 있습니다.
      auth.requestMatchers("/admin/**").hasAnyAuthority(UserRole.USER.getUserRole());  // "/admin"으로 시작하는 주소는 "ADMIN" userRole을 가진 사용자가 접근할 수 있는 주소입니다.
